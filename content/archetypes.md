...
looks like and what comes back. Five labels cover all 324 topics, which means
...
| --- | --- | --- | --- |
| `record-transform` | 257 | domain-specific record or array | domain-specific result |
| `series-transform` | 37 | `(number \| null)[]` | array of the same length |
| `row-classify` | 17 | array of rows | one verdict per row |
| `tape-aggregate` | 7 | `Trade[]` | `Bar[]` |
| `snapshot-evaluate` | 6 | one point-in-time snapshot | one verdict |
...
thirteen domains and 79% of the library.
...
the 324 has a parameter table with types, constraints and null handling, and an
...
metadata, not a load-bearing type. It is there to narrow 324 candidates to a
...
The counts on this page are current as of the package version named at the foot
of the reference pages. For the live figures, fetch
[version.json](/version.json) or the
[reference payload](/reference/payload.json) and group by `import.archetype`.
